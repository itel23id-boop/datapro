...
class getIrvankedeSMM extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get:irvankede';
    ...
    public function handle()
    {
        ...
        $irvankede = new IrvankedesmmController;
        $res = $irvankede->service();
        
        $api = \DB::table('setting_webs')->where('id',1)->first();
        if($api->irvankede_status == "Active") {
            foreach(Kategori::get() as $kategori){
                foreach($res['data'] as $data){
                    if($kategori->provider == "irvankedesmm") {
            ...
            $output = [0 => ['result' => false,'data' => 'Failed to retrieve IRVANKEDESMM data with inactive status']];
        }
