...
class LinkQuCallbackController extends Controller
{
    ...
    public function __construct()
    {
        ...
        /* LINKQU USER */
        $this->linkqu_username = $api->linkqu_username;
    }
    ...
    public function handle(Request $request)
    {
        ...
        if (isset($data->status,$data->partner_reff,$data->username)){
            $order_id = $data->partner_reff;
            
            $pembayaran = Pembayaran::where('order_id', $order_id)->where('status', 'Menunggu Pembayaran')->first();
            
            if ($data->username === $this->linkqu_username) {
                        ...
                        if($pembayaran->type == 'deposit') {
                            ...
                            $dataPembeli->minimal, $dataPembeli->maximal, $dataPembeli->post, $dataPembeli->old_post, $dataPembeli->delay, $dataPembeli->expiry);
                                ...
                                if($this->status_irvankede == "Active") {
                                    $partaisocmed = new IrvankedesmmController;
                                    $order = $partaisocmed->order($dataPembeli->user_id, $dataLayanan->provider_id, $dataPembeli->quantity, $dataPembeli->custom_comments, $dataPembeli->usernames, $dataPembeli->hashtag, $dataPembeli->media, $dataPembeli->answer_number);
                ...
                return Response::json(['error'=>"Invalid Username"])->setStatusCode(410);
            }
        } else {
