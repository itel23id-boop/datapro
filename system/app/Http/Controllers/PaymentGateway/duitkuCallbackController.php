...
class duitkuCallbackController extends Controller 
{
    public function handle(Request $request)
    {
        ...
        try {
            $apiKey = isset($api->duitku_apikey) ? $api->duitku_apikey : '-';
            ...
            $spUserHash = isset($request->spUserHash) ? $request->spUserHash : null; // Shopee only
            ...
            if (!empty($merchantCode) && !empty($amount) && !empty($merchantOrderId) && !empty($signature)) {
                ...
                $pembayaran = Pembayaran::where('reference', $reference)->where('status', 'Menunggu Pembayaran')->first();
                        ...
                        if($pembayaran->type == 'deposit') {
                            ...
                            $dataPembeli->minimal, $dataPembeli->maximal, $dataPembeli->post, $dataPembeli->old_post, $dataPembeli->delay, $dataPembeli->expiry);
                                ...
                                if($api->vipmember_status == "Active") {
                                    $vipmember = new vipmember_status;
