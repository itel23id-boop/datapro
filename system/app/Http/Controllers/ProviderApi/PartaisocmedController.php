...
class PartaisocmedController extends Controller
{
    private $base = 'https://partaisocmed.my.id/api/v2';
    private $key;
    
    public function __construct() {
        $api = \DB::table('setting_webs')->where('id',1)->first();
        $this->key = $api->partaisosmed_key;
    }
    
    public function order($target = null, $service = null, $quantity = null, $runs = null, $interval = null, $keywords = null, $comments = null, $usernames = null, $hashtag= null, $username = null, $media = null, $answer_number = null,
    $min = null, $max = null, $posts = null, $old_posts = null, $delay = null, $expiry = null)
    {
        $data = $this->connect($this->base,array(
            ...
            'username' => $username,
            'media' => $media,
            'min' => $min, // for subscribe
            'max' => $max, // for subscribe
            'posts' => $posts, // optional for subscribe
            'old_posts' => $old_posts, // for subscribe
            'delay' => $delay, // for subscribe
            'expiry' => $expiry, // optional for subscribe
        ));
    ...
    public function service()
    {
        ...
        for($i = 0; $i <= count($data)-1; $i++) {
            $out[] = [
                ...
                'min' => $data[$i]['min'],
                'max' => $data[$i]['max'],
                'price' => $formatter->kurs($data[$i]['rate']),
    ...
    public function CheckBalance()
    {
        ...
        if(isset($data['success'])) {
            ...
            return ['result' => true,'data' => ['balance' => $formatter->kurs($data['balance'])],'message' => 'Balance successfully obtained.'];
        }
    }
