    ...
    <div class="container">
        <div class="row mb-4">
            <div class="col-md-12">
                ...
                <div class="owl-carousel owl-theme" id="artikel-banner">
                    <!-- start foreach -->
                    ...
                    @endphp
                    <div class="item">
                        <a href="/artikel/{{$datas->url}}" class="d-inline-block w-100 text-decoration-none">
                            <div class="artikel-banner-card" style="background-image: url({{$datas->path}});">
                                <div class="artile-banner-card-body">
                                <span class="category mb-2">{{$datas->category}}</span>
                                <h4 class="fw-600 lh-32">{{$datas->title}}</h4>
                                    <div class="artikel-data text-white">
                ...
                <div class="owl-carousel owl-theme" id="artikel-banner-mobile">
                    <!-- start foreach -->
                    @foreach($data as $datas)
                    @php
                    $excerpt_lengths = 30;
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="mb-4" id="searchBerita">
                    <div class="input-group">
                        <input type="search" name="slug" id="slug" class="form-control border-0" autocomplete="off" placeholder="Cari Berita" value>
                    </div>
                    <div class="inset-x-0 rounded-lg rounded-t-none bg-primary shadow-md md:rounded-t-lg" style="max-height: 600px;overflow:auto;margin: 1rem;" id="live_search_artikel"></div>
                </div>
                <div class="mb-4">
...
<script>
    ...
    let input = $('#slug')
    let base = $('#live_search_artikel')
    base.removeClass('d-none')
        if(input.val()){
                $.ajax({
                    method: "post",
                    url: "{{url('/artikel/cari')}}",
                    headers: {
                        ...
                        $.each(categories, function (index, value) { 
                            html += `<div class="space-y-4 p-3">
            	                       <a class="block" href="${value.url}">
            	                           <div class="items-center gap-x-3" style="display: flex;flex-wrap: wrap;flex-direction: column;">
            	                               <div class="relative overflow-hidden rounded-lg" style="width: 20rem;height: 10rem;">
            	                                   <img alt="${value.title}" loading="lazy" decoding="async" data-nimg="fill" sizes="100vw" src="${value.path}" style="position: absolute; height: 100%; width: 100%; inset: 0px; color: transparent;">
            	                               </div>
                	                           <div>
                	                               <h3 class="font-semibold" style="line-height: 30px!important;margin: 0!important;padding-top: 1rem;">${value.title}</h3>
                	                               <h5 class="text-xxxs text-secondary-foreground">${value.category}</h5>
                	                           </div>
            	                           </div>
            	                        </a>
            	                     </div>`
                    ...
                    error: function(response){
                        base.html(`<div class="space-y-4 p-3">
            	                       <div class="flex items-center gap-x-3">
                	                       <div>
                	                           <h3 class="text-xxs" style="line-height: 30px!important;margin: 0!important;">Artikel tidak ada!</h3>
                	                       </div>
            	                       </div>
            	                    </div>`)
                }
...
$('#artikel-banner-mobile').owlCarousel({
    loop:true,
    margin:10,
    nav:true,
    responsive:{
