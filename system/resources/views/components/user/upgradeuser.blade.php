			...
			<div class="row">
				...
				<div class="col-md-8">
					<div class="card border-0 shadow-form">
						<div class="card-body">
						    <a href="/user/upgrade/riwayat" class="text-decoration-none float-end mt-2">
								<i class="fa fa-history me-1"></i> Data Riwayat
							</a>
							<h6 class="card-title">Upgrade Akun</h6>
							@if(session('error'))
                            <div class="alert alert-danger">
                                {{ session('error') }}
                            </div>
                            @endif
                            @if(in_array(Auth::user()->role, ["Member","Reseller"]))
							<form action="{{ route('upgrade.post') }}" method="POST" id="form-upgrade">
								...
								<div class="mb-3">
									<select class="form-control" name="role" id="role">
                                        ...
                                        <option value="Reseller">Reseller (Rp. {{number_format($setting->harga_upgrade_reseller, 0, '.', ',')}})</option>
                                        <option value="VIP">VIP (Rp. {{number_format($setting->harga_upgrade_vip, 0, '.', ',')}})</option>
									...
									<div class="accordion mb-2" id="accordionExample">
									    ...
									    <div class="border-0 mb-3">
									        <h2 class="accordion-header" id="heading-6">
									            <button style="background: var(--warna_5);border-radius: 6px 6px 0 0;" class="accordion-button collapsed text-white" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-6" aria-expanded="false" aria-controls="collapse-1">Bank Transfer (Manual Otomatis)</button>
									        </h2>
								...
								<div class="text-end">
									...
									<button class="btn btn-primary" type="button" id="btn-upgrade" onclick="topup_process();">Isi Saldo</button>
								</div>
							</form>
							@else
                            <div class="alert alert-danger">
                                Maaf anda tidak bisa menggunakan fitur upgrade ini!
                            </div>
                            @endif
						</div>
					</div>
...
<script type="text/javascript">
    function select_method(id) {
		var level = $("select[name=role]").val();

		if (!level) {
			toastr.error('Silahkan pilih role terlebih dahulu!', 'Gagal');
		} else {
	...
	$("#role").change(function() {
	    $.ajax({
			url: "<?php echo route('upgrade.price') ?>",
			type: "POST",
			data: {
    ...
    function topup_process() {
		...
		if (!level) {
			...
			$("#btn-upgrade").text('Loading...').attr('disabled', 'disabled');
			setTimeout(function() {
				$("#form-upgrade").submit();
			}, 1500);
		}
